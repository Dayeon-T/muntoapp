...
export default function SocialingCard({
  ...
  const { totalParticipants, totalAttended, totalNoShow, totalRegistered } =
    stats;
  ...
  return (
    <div
      className={`relative rounded-xl border bg-white border-slate-200 overflow-hidden transition-all duration-200 ${
        isInactive
          ? "opacity-60"
          : showCancelModal
          ? ""
    ...
    >
        ...
        <div className="flex flex-wrap items-center gap-1.5 mt-2">
          <span
            className={`text-[11px] font-medium py-0.5 ${
              totalNoShow > 0 ? "text-red-600" : "text-slate-600"
            }`}
          >
            {getParticipantText()}
          </span>
      ...
      >
        <div className="border-t border-slate-100 px-4 py-3 space-y-3">
          ...
          <div>
            ...
            <ul className="space-y-1">
              {event.participants.map((p) => (
                <li
                  key={p.id}
                  className="text-[11px] text-slate-600 flex items-center justify-between"
                >
            ...
            <div className="pt-2 border-t border-slate-100 flex items-center justify-between">
              <p className="text-[11px]">
                <span className="text-slate-500">
                  {[event.hasAlcohol && "음주", event.isNight && "야간"]
                    .filter(Boolean)
                    .join(" · ")}
                </span>
          ...
          {/* 수정 버튼 (예정/완료 모두 가능, 취소된 건 제외) */}
          {event.status !== "cancelled" && onEditClick && (
            <div
              className={`pt-2 ${
                needsAttention ? "" : "border-t border-slate-100"
              }`}
            >
              <button
                type="button"
          ...
          {/* 예정된 이벤트: 취소 버튼 */}
          {event.status === "scheduled" && onCancelEvent && (
            <div className="pt-2">
              <button
                type="button"
                onClick={(e) => {
